...
def export_gt_depths_kitti():
    ...
    parser.add_argument('--split',
                        ...
                        required=True,
                        choices=["eigen", "eigen_benchmark", "kitti"])
    ...
    for line in lines:
        ...
        elif opt.split == "kitti":
            calib_dir = os.path.join(opt.data_path, 'calibration')
            velo_filename = os.path.join(opt.data_path, folder,
                       "velodyne_points/data", "{:010d}.bin".format(frame_id))
            gt_depth_path = os.path.join(opt.data_path, folder,
                                         "velodyne_points/data", "{:010d}.bin".format(frame_id))
            gt_depth = generate_depth_map(calib_dir, velo_filename, sides[side], True)
